...
def condition_info_nce_for_embeddings_fairmi_star(x, z, tau=0.5):
    N = x.shape[0]
    x_norm = F.normalize(x)
    z_norm = F.normalize(z)

    pos_score = torch.sum(x_norm * z_norm, dim=1)
    pos_score = torch.exp(pos_score / tau)

    neg_score = torch.exp(torch.mm(x_norm, z_norm.T) / tau).sum(dim=1) - pos_score  # Vectorized computation

    cl_loss = -torch.log(pos_score / (neg_score + 1e-10)).mean()  # Adding a small constant for numerical stability
    return cl_loss
...
def train_unify_mi(sens_enc, inter_enc, club, dataset, u_sens,
    ...
    for epoch in range(args.num_epochs):
        ...
        for uij in train_loader:
            ...
            lb1 = condition_info_nce_for_embeddings_fairmi_star(e_xu[torch.unique(u)], e_zu[torch.unique(u)])   
            ...
            # up = args.ureg * club.forward(e_zu[torch.unique(u)], e_zi[torch.unique(i)])
            # Generate random embeddings once per epoch
            embedding_dim = e_zu.shape[1]  # Assuming the second dimension is the embedding size
            num_samples = e_zu.shape[0]  # This should match the batch size of e_zu

            # Generate random embeddings and normalize them
            y_samples = torch.randn(num_samples, embedding_dim).to(e_zu.device)
            y_samples = torch.nn.functional.normalize(y_samples, p=2, dim=1)
        
            up = args.ureg * club.forward(e_zu[torch.unique(u)], y_samples[torch.unique(u)])
        ...
        # y_samples = e_zi.detach()  # Non-sensitive embeddings
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_path', type=str, default='logs/bpr_fairmi_star.txt')
    parser.add_argument('--param_path', type=str, default='param/bpr_fairmi_star.pth')
